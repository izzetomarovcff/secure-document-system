...
function Userssubadmin() {
  const [users, setUsers] = useState(null)
  const [permissions, setPermissions] = useState(null)
  const token = localStorage.getItem('token');
  const { role_id, administration_id,department_id } = JSON.parse(decodeURIComponent(escape(atob(token.split('.')[1]))));
      ...
      <div className='group'>
        ...
        <div className="containerr">
          <div className="head-group">
            ...
            {permissions && permissions.has("add_user") ? (<Link to={"/subadmin/users/create"} className="lnk-btn btn btn-secondary">Istifadəçi Yarat</Link>) : (<button disabled className="lnk-btn btn btn-secondary">Istifadəçi Yarat</button>)}
          </div>
                        ...
                        return (
                          <tr key={key}>
                            ...
                            <td>
                              ...
                              {permissions && permissions.has("edit_user") ? (<Link to={`/subadmin/users/edit/${element.id}`} className='btn btn-sm btn-primary me-3'>Dəyişdir</Link>) : (<button disabled className='btn btn-sm btn-primary me-3'>Dəyişdir</button>)}
